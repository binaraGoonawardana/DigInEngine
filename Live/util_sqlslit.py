...
def sqlsplit(dbname,policyClient,field1,field2,field3,delimiter):
    ...
    # delimiter = "' '"
    engine = sql.create_engine("mssql+pyodbc://smsuser:sms@192.168.1.83:1433/HutchDialogic?driver=SQL+Server+Native+Client+11.0")
    ...
    query = "SELECT	{5},str({3},8,2) val1,{4} val2,dbo.split({0}, ' ', (len({0}) - len(replace({0}, ' ', '')) + 1)) city1,\
            dbo.split({0}, ' ', (len({0}) - len(replace({0}, ' ', '')))) city2,\
            dbo.split({0}, ' ', (len({0}) - len(replace({0}, ' ', '')) - 1)) city3,\
            dbo.split({0}, ' ', (len({0}) - len(replace({0}, ' ', '')) - 2)) city4\
