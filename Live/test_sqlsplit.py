...
def sqlsplit(dbname,policyClient,field1,field2,field3,delimiter):
    ...
    query = "SELECT	{5} policyClient,str({3},8,2) val1,{4} val2,dbo.split({0}, ' ', (len({0}) - len(replace({0}, ' ', '')) + 1)) city1,\
...
# sqlsplit('DimPolicy','policyClient','ClientAddress','SumInsured','CoverType',"' '")